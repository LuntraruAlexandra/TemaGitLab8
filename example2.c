...
int is_empty(NODE *queue) { return queue == NULL; }
...
void BFS(GPH *graph, int start) {
    ...
    while (!is_empty(queue)) {
        ...
        while (temp != NULL) {
            ...
            if (graph != NULL && graph->visited[adj_vertex] == 0) {
...
/*
void afisare_graf(GPH *graf)
{
    printf("Liste adiacenta:\n");
    for(int i = 0; i <= 4; i++)
    {
        NODE *p = graf->adjacency_lists[i];
        while(p!=NULL)
        {
            printf("%d->%d\n",i,p->data);
            p = p->next;
